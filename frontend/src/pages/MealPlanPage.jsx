...
export const MealPlanPage = () => {
    ...
    const loadSaved = useCallback(async () => {
        try { setSaved(await listMealPlans(getDeviceId())); }
    ...
    const onDelete = async (id) => {
        try { await deleteMealPlan(id, getDeviceId()); toast.success(t("plans.deleted")); loadSaved(); }
        catch { toast.error("Error"); }
    };
    ...
    return (
        <div className="min-h-screen pb-32 px-5 pt-10">
            ...
            <div className="flex gap-2 mb-6">
                {[["nuovo", t("plans.new")], ["salvati", `${t("plans.saved")} (${saved.length})`]].map(([id, label]) => (
                    <button
                        key={id}
